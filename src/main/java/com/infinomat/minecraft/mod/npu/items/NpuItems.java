...
public class NpuItems {
    ...
    public static final HashMap<String, List<DeferredItem<Item>>> CreativeModeTabMap = new HashMap<>();    // 物品栏-物品映射
    ...
    public static List<DeferredItem<Item>> getItemList(String tabName) {
        return CreativeModeTabMap.get(tabName);
    }
    ...
    private static void register() {
        for (var creativeModeTab : RegisterListMap.keySet()) {
            CreativeModeTabMap.put(creativeModeTab, new ArrayList<>());
            for (var group : RegisterListMap.get(creativeModeTab).getGroups()) {
                CreativeModeTabMap.get(creativeModeTab).addAll(TemplateMap.get(group.template).register(group.items));
            }
        }
        for (var creativeModeTab : NpuBlocks.CreativeModeTabMap.keySet()) {
            CreativeModeTabMap.computeIfAbsent(creativeModeTab, k -> new ArrayList<>());
            NpuBlocks.CreativeModeTabMap.get(creativeModeTab).forEach(block -> CreativeModeTabMap.get(creativeModeTab)
                    .add(ITEMS.register(
